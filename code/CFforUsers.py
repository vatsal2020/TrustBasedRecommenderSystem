...
def predictrating(user,item):
    ...
    if(len(posit_corrs)+len(neget_corrs)<=2):
        return -1
    return user_averages[user] - (np.dot(posit_corrs, rel_rating[posit_vals]) + 
     np.dot(neget_corrs,6 - rel_rating[neget_vals]))/(sum(posit_corrs) + sum(abs(neget_corrs)))
